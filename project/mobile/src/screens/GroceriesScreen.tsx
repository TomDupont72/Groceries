...
import { useTheme, Card, List, ListItem, ListItemInput, ListItemText, Checkbox, Button, Badge } from "../theme/index"
import { useGroceries } from "../hooks/useGroceries";
import LottieView from "lottie-react-native";

export default function GroceriesScreen() {
  const { theme } = useTheme();
  const { loadingAddGrocery, loadingPage, loadingRefresh, errorMsg, recipeData, selectedRecipes, groceryRecipeData, toggleRecipe, setQuantity, submit, loadAll } = useGroceries();
    ...
    <SafeAreaView style={[styles.safeArea, { backgroundColor: theme.colors.bg }]}>
      <ScrollView contentContainerStyle={[styles.section, { paddingBottom: theme.spacing.xl * 3 }]}>
        ...
        <Card variant="outlined" padding="md" style={styles.section}>
          <List header="Recettes" columns={[20, "flex", 60]}>
            {recipeData.map((item) => (
              <ListItem key={item.id}>
                <Checkbox checked={item.id in selectedRecipes} onPress={() => toggleRecipe(item.id)}/>
                <ListItemText>{item.name}</ListItemText>
                <ListItemInput value={selectedRecipes[item.id]} onChangeText={(txt) => setQuantity(item.id, txt)} placeholder="Qté" keyboardType="numeric" disabled={!(item.id in selectedRecipes)}/>
              </ListItem>
            ))}
          </List>
          {errorMsg ? (
          ...
          <Button title="Ajouter aux courses" onPress={submit} fullWidth loading={loadingAddGrocery}/>
        </Card>
